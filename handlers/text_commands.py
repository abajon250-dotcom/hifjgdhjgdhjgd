...
async def cmd_balance(message: types.Message):
    ...
    db.get_user(uid)
    await message.answer(f"{DOLLAR} Баланс: <b>{db.get_balance(uid):.2f}</b>",
                         parse_mode="HTML")


@router.message(F.text.regexp(r"(?i)^(игры|играть|и|games|game)$"))
async def cmd_games(message: types.Message):
    await message.answer("🎮 Выбирайте игру:", reply_markup=games_main())
...
async def cmd_help(message: types.Message):
    ...
    await message.answer(
        ...
        "• <code>ставка</code> — показать текущую\n"
        "• <code>вб</code> — поставить ВЕСЬ баланс\n"
        "• <code>деп 5</code> — пополнить на 5 USDT\n"
        "• <code>вывод 5</code> — вывести 5 USDT\n\n"
        "🎲 <b>Игры:</b>\n"
        "• <code>куб 5</code> — кубик на число 5\n"
        "• <code>куб 3,4</code> — кубик на 2 числа\n"
        "• <code>куб 7-</code> — 2 куба, сумма < 7\n"
        "• <code>куб 7+</code> — 2 куба, сумма > 7\n"
        "• <code>куб 7</code> — 2 куба, сумма = 7\n"
...
async def cmd_profile(message: types.Message):
    ...
    await message.answer(
        ...
        f"📉 Оборот: <b>{s['total_wagered']:.2f}</b>\n"
        f"🎲 Игр: <b>{s['games_played']}</b>\n"
        f"🤝 Приглашено: <b>{s['invited_count']}</b>",
        parse_mode="HTML"
    )
...
async def cmd_bet_show(message: types.Message):
    await message.answer(
        f"🎯 Текущая ставка: <b>{get_bet(message.from_user.id)}</b> {DOLLAR}",
        parse_mode="HTML"
    )
...
async def cmd_bet_set(message: types.Message):
    ...
    await message.answer(f"✅ Ставка: <b>{bet}</b> {DOLLAR}", parse_mode="HTML")
