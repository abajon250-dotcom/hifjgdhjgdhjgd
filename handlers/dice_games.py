...
def _label(choice):
    if choice in CHOICE_LABEL: return CHOICE_LABEL[choice]
    if choice.startswith("num"): return f"число {choice[3:]}"
    if choice.startswith("exact_"): return f"точное {choice.split('_')[1]}"
...
async def _bet_message(target, uid, bet, game_label):
    ...
    await target.answer(f"🎲 {mention} поставил <b>{bet:.2f}$</b> на <b>{game_label}</b>",
                        parse_mode="HTML")
...
async def d1_allin(call: types.CallbackQuery):
    ...
    if bal <= 0: return await call.answer("❌ Нет баланса!", show_alert=True)
    db.set_bet(uid, bal)
    await call.message.edit_text(
        f"💥 <b>ВБ установлен: {bal:.2f}</b> {DOLLAR}\n\n{DICE} Выберите исход игры:",
        reply_markup=dice_menu_1(uid), parse_mode="HTML")
...
async def play_1(call: types.CallbackQuery):
    if call.data.startswith("d1:allin"): return
...
async def play_1_two(call: types.CallbackQuery):
    ...
    db.inc_games(uid)
    await _bet_message(call.message, uid, bet, f"числа {','.join(map(str, nums))}")
    await call.answer()
    ...
    if v in nums: win, result = round(bet * 2.8, 2), "win"
    else: win, result = 0, "lose"
...
async def _play(call, uid, dtype, choice):
    bet = db.get_bet(uid)
    if not db.has_enough(uid, bet):
        return await call.answer(f"❌ Нужно {bet} 💵. Баланс: {db.get_balance(uid):.2f}",
                                 show_alert=True)
    db.update_balance(uid, -bet)
    db.add_wager(uid, bet)
    db.inc_games(uid)
...
async def _finish(call, uid, game_key, value, bet, win, result, mult, choice=""):
    ...
    if result == "win":
        ...
        except Exception as e: print(f"notify_win: {e}")
    else:
        extra = -win if win < 0 else 0.0
        total_loss = bet + extra
        if extra > 0: db.update_balance(uid, -extra)
        ...
        except Exception as e: print(f"notify_lose: {e}")


async def play_dice_direct(message: types.Message, dtype: int, choice: str, uid: int = None):
