...
async def _bet_msg(target, uid, bet):
    ...
    await target.answer(f"🎰 {mention} поставил <b>{bet:.2f}$</b> на <b>слоты</b>",
                        parse_mode="HTML")
...
async def slots_play(call: types.CallbackQuery):
    ...
    for _ in range(3):
        ...
        try: await msg.edit_text(f"🎰 | {a} | {b} | {c} |")
        except Exception: pass
        await asyncio.sleep(0.4)
    reels = spin_slots()
    try: await msg.edit_text(f"🎰 | {reels[0]} | {reels[1]} | {reels[2]} |")
    except Exception: pass
    await asyncio.sleep(0.5)
    try: await notify_dice(call.bot, uid, msg)
    except Exception: pass
...
async def play_slots_direct(message: types.Message, choice: str = "any", uid: int = None):
    ...
    await asyncio.sleep(0.5)
    try: await notify_dice(message.bot, uid, msg)
    except Exception: pass
    win, result = calc_slots(bet, choice, reels)
    reels_str = f"{reels[0]} {reels[1]} {reels[2]}"
    mult = win / bet if bet and win > 0 else 0
