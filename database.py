...
class Database:
    ...
    def get_games_stats(self, uid):
        ...
        for row in self.cursor.fetchall():
            gtype, games, wins, losses, won, staked = row
            out[gtype] = {
                "games": games or 0,
                "wins": wins or 0,
                "losses": losses or 0,
                "won": round(won or 0, 2),
                "staked": round(staked or 0, 2),
    ...
    def get_admin_stats(self, day_start):
        ...
        total_turnover = self.cursor.fetchone()[0]

        self.cursor.execute("SELECT COALESCE(SUM(balance),0) FROM users")
        total_balances = self.cursor.fetchone()[0]
